...
import { normalise, has, MOD_CHECKS, evalMod, evalMake, evalExt } from './checkers.js';
...
describe('MOD_CHECKS.mod4', () => {
  ...
  test('fails when print_line not defined', () => {
    const r = evalMod('mod4', 'def hello():\n    print("Hi")\nhello()');
    expect(r.pass).toBe(false);
...
describe('validateMake', () => {
  test('passes a complete valid program', () => {
    const r = evalMake(VALID_MAKE);
    expect(r.pass).toBe(true);
  });

  test('fails when program is empty', () => {
    expect(evalMake('').pass).toBe(false);
  });

  test('fails the "5 functions in total" requirement when fewer than 5 are defined', () => {
    // Checked directly via r.results[5] (rather than r.msg) since with fewer than
    // 5 functions overall, an earlier bullet (e.g. animal functions) fails first —
    // this snippet's missing animal functions ARE also why it's short of 5 total.
    const r = evalMake(
      'def show_menu(): print("Menu")\nchoice = input("Enter: ")\nif choice == "1": pass\nelif choice == "2": pass\nelse: print("Sorry, that\'s not a valid option.")'
    );
    expect(r.pass).toBe(false);
    expect(r.results[5]).toBe(false);
  });

  test('fails when input() is missing', () => {
    const noInput = VALID_MAKE.replace('input("Enter 1, 2 or 3: ")', '"1"');
    const r = evalMake(noInput);
    expect(r.pass).toBe(false);
    expect(r.msg).toContain('input()');
  });

  test('fails when show_menu is missing', () => {
    const r = evalMake(VALID_MAKE.replace('def show_menu', 'def start_menu'));
    expect(r.pass).toBe(false);
    expect(r.msg).toContain('show_menu');
  });

  test('fails when show_goodbye is missing', () => {
    const r = evalMake(VALID_MAKE.replace('def show_goodbye', 'def end_program'));
    expect(r.pass).toBe(false);
...
describe('validateExt', () => {
  test('passes with while loop and break', () => {
    expect(evalExt(VALID_EXT).pass).toBe(true);
  });

  test('fails when program is too short', () => {
    expect(evalExt('while True: pass').pass).toBe(false);
  });

  test('fails when while loop is missing', () => {
    const r = evalExt(VALID_EXT.replace('while True:', 'if True:'));
    expect(r.pass).toBe(false);
    expect(r.msg).toContain('while');
  });

  test('fails when break is missing', () => {
    const r = evalExt(VALID_EXT.replace('break', 'pass'));
    expect(r.pass).toBe(false);
